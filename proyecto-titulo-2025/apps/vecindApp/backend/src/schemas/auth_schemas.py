...
class UsuarioRegistroRequest(BaseModel):
    ...
    # Datos del vecino
    rut: str = Field(
        ..., min_length=8, max_length=12, description="RUT sin puntos ni guión"
    )
    nombres: str = Field(
        ..., min_length=2, max_length=100, description="Nombres del vecino"
    )
    apellido_paterno: str = Field(
        ..., min_length=2, max_length=100, description="Apellido paterno"
    )
    apellido_materno: str = Field(
        ..., min_length=2, max_length=100, description="Apellido materno"
    )
    ...
    direccion: str = Field(
        ..., min_length=5, max_length=200, description="Dirección del vecino"
    )
    foto_perfil: Optional[str] = Field(
        None, description="Foto de perfil codificada en base64 (opcional)"
    )
    ...
    def validate_email_format(cls, v):
        """
        Validación adicional del email para formato correcto.
        """
        if not v:
    ...
    def validate_phone(cls, v):
        ...
        if len(phone_digits) != 9:
            raise ValueError(
                "El teléfono debe tener exactamente 9 dígitos después de +56"
            )
    ...
    def validate_photo(cls, v):
        ...
        try:
            ...
            if not v.startswith("data:image/"):
                raise ValueError(
                    "La foto debe ser una imagen en formato base64 con prefijo data:image/"
                )

            # Extraer el tipo MIME y los datos
            if "," not in v:
                raise ValueError(
                    "Formato de imagen base64 inválido: falta la coma separadora"
                )
            ...
            if not data or len(data.strip()) < 10:
                raise ValueError(
                    "Los datos base64 de la imagen están vacíos o son muy cortos"
                )
...
class LoginRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"email": "juan.perez@gmail.com", "password": "MiPass123!"}
        }
